...
Price data can be displayed on a chart in multiple ways — from standard **candlestick charts** to specialized formats like **Renko** or **Heikin Ashi candles**. Each type filters and presents information differently. This lesson covers the specifics of each chart type and what distinguishes them.
...
Japanese candlesticks are covered in the previous lesson — this section is a quick recap. Candlesticks are the most popular charting method for technical analysis. The anatomy consists of the **body** and the **wick**. A candle closing above its open is bullish; closing below is bearish. Wicks mark the highest and lowest points of the trading range.

### Time-Based Charts

The two charts below look identical — but they are not. The chart on the left is time-based; the chart on the right is not. **Chart periodicity** is the primary setting when configuring a candlestick chart. Time-based periodicity is the most widely used option. Most trading platforms support minutes, hours, days, weeks, and seconds.
...
> **Tip:** Using standard timeframes is effective because many traders are watching the same levels. A non-standard chart — such as 40-minute or 3-hour — produces candlestick patterns that fewer traders observe, which reduces their significance.

### Non-Time-Based Charts

Time is not the only way to structure a chart. **Tick and range charts** are the most popular non-time-based alternatives available in the FTMO cTrader platform.
...
A new brick appears when the market moves more than the **brick size** away from the previous brick. If one brick represents a 5-pip movement, no new brick forms until price travels those 5 pips in one direction from the last brick's close. This makes Renko charts effective for filtering out noise and for identifying [support and resistance](/lesson/support-and-resistance/) areas.
